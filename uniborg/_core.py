...
async def load_reload(event):
    ...
    try:
        ...
        msg = await event.respond(f"Innstalling Successfully (re)load plugin {shortname}")
...
async def send_plug_in(event):
    ...
    await event.edit("plugin is uploading  {} in {} seconds".format(input_str, time_taken_in_ms))
    await asyncio.sleep(DELETE_TIMEOUT)
    await event.delete()
    ...
    if event.reply_to_msg_id:
        try:
            ...
            if "(" not in downloaded_file_name:
                ...
                await event.edit("plugin is removed!!")
        except Exception as e:  # pylint:disable=C0103,W0703
